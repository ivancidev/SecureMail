...
const debugEnabled =
  (process.env.DEBUG || "").toLowerCase() === "true" || isDev;
const logDebug = (...args) => {
...
const userSchema = new mongoose.Schema({
  ...
  role: {
    type: String,
    enum: ["admin", "manager", "team-leader", "agent", "bde"],
    required: true,
    default: "agent",
  },
...
const PermanentEmailDeletion = mongoose.model(
  "PermanentEmailDeletion",
  permanentEmailDeletionSchema
);
const DistributionList = mongoose.model(
  "DistributionList",
    ...
    if (userCount === 0) {
      const sampleUsers = [
        ...
        },
        {
          ...
          isActive: true,
          avatar:
            "https://images.unsplash.com/photo-1519244703995-f4e0f30006d5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facepad&facepad=2&w=256&h=256&q=80",
        },
        {
          id: "agent-1",
          firstName: "Jane",
          ...
          isActive: true,
          avatar:
            "https://images.unsplash.com/photo-1494790108755-2616b612b786?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facepad&facepad=2&w=256&h=256&q=80",
        },
        {
          id: "manager-2",
          firstName: "Bob",
          ...
          isActive: true,
          avatar:
            "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facepad&facepad=2&w=256&h=256&q=80",
        },
        {
          id: "agent-2",
          firstName: "Alice",
          ...
          isActive: true,
          avatar:
            "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facepad&facepad=2&w=256&h=256&q=80",
        },
      ];
  ...
  try {
    ...
    if (requestingUser.role !== "admin" && requestingUser.role !== "manager") {
      return res.status(403).json({
        error: "Only administrators and managers can view activity reports",
      });
    }
...
app.get("/api/users/attachment-permissions", async (req, res) => {
  try {
    ...
    logDebug(
      `[Attachment Permissions] Request received with adminUserId: ${adminUserId}`
    );
    ...
    logDebug(
      `[Attachment Permissions] Looking for admin user with id: ${adminUserId}`
    );
    ...
    if (adminUser.role !== "admin") {
      logDebug(
        `[Attachment Permissions] User is not admin, role: ${adminUser.role}`
      );
      return res
        .status(403)
        .json({ error: "Only administrators can view attachment permissions" });
    }
    ...
    const usersWithPermissions = users.map((user) => ({
      ...
      userRole: user.role,
      canSendAttachments:
        user.role === "admin" && user.canSendAttachments === undefined
          ? true
          : user.canSendAttachments || false,
    }));
...
app.put("/api/users/:id/reset-password", async (req, res) => {
  try {
    ...
    if (adminUser.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can reset passwords" });
    }
...
app.get("/api/users/:id/attachment-permission", async (req, res) => {
  try {
    ...
    res.json({
      canSendAttachments:
        canSendAttachments !== undefined ? canSendAttachments : false,
      userId: user.id,
      userName: `${user.firstName} ${user.lastName}`,
      userEmail: user.email,
      userRole: user.role,
...
app.put("/api/users/:id/attachment-permission", async (req, res) => {
  try {
    ...
    if (canSendAttachments === undefined || !adminUserId) {
      return res
        .status(400)
        .json({ error: "canSendAttachments and adminUserId are required" });
    }
    ...
    if (adminUser.role !== "admin") {
      return res.status(403).json({
        error: "Only administrators can modify attachment permissions",
      });
    }
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Attachment permissions can only be set for Admin, Manager, Team Leader, and BDE roles",
      });
    }
...
app.post("/api/users/:userId/login-history", async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error:
          "Only administrators and managers can modify other users login history",
      });
    }
...
app.delete("/api/users/:userId/login-history/:entryId", async (req, res) => {
  try {
    ...
    if (adminUser.role !== "admin" && adminUser.role !== "manager") {
      return res.status(403).json({
        error: "Only administrators and managers can modify user login history",
      });
    }
...
app.get("/api/emails", async (req, res) => {
  try {
    ...
    logDebug(
      `[API Request] /api/emails - userId: ${userId}, archived: ${archived}`
    );
    ...
    const userPermanentDeletions = await PermanentEmailDeletion.find({
      userId: userId,
    });
    const permanentlyDeletedEmailIds = userPermanentDeletions.map(
      (deletion) => deletion.emailId
    );
    logDebug(
      `[API Debug] Permanently deleted email IDs:`,
      permanentlyDeletedEmailIds
    );
    ...
    if (archived === "true") {
      ...
      logDebug(
        `[API Debug] Email IDs found:`,
        emails.map((e) => e.id)
      );

      // Filter to only include emails that are archived by this user
      filteredEmails = emails.filter((email) =>
        archivedEmailIds.includes(email.id)
      );

      // Filter out emails that this user has permanently deleted
      filteredEmails = filteredEmails.filter(
        (email) => !permanentlyDeletedEmailIds.includes(email.id)
      );
      logDebug(
        `[API Debug] After filtering for archived: ${filteredEmails.length} emails`
      );
    } else {
    ...
    if (newEmail.recipientIds && newEmail.recipientIds.length > 0) {
      ...
      newEmail.recipientIds.forEach((recipientId) => {
        io.to(`user-${recipientId}`).emit("new-email", {
          ...
          bodyPreview:
            newEmail.body.substring(0, 100) +
            (newEmail.body.length > 100 ? "..." : ""),
        });
      });
    }
...
app.delete("/api/emails/:id", async (req, res) => {
  try {
    ...
    if (existingDeletion) {
      return res
        .status(400)
        .json({ error: "Email already deleted by this user" });
    }
...
app.post("/api/emails/:id/archive", async (req, res) => {
  try {
    ...
    if (existingArchive) {
      return res
        .status(400)
        .json({ error: "Email already archived by this user" });
    }
...
app.get("/api/emails/:emailId/read-receipts", async (req, res) => {
  try {
    ...
    const recipientsByEmail = await User.find({
    ...
    const externalRecipients = emailAddresses
      .filter(
        (email) => !recipientsByEmail.some((user) => user.email === email)
      )
      .map((email) => ({
        id: email,
        firstName: "External",
    ...
    const readReceiptData = allRecipientsWithExternal.map((recipient) => {
      const readRecord = emailReads.find(
        (read) => read.userId === recipient.id
      );
      return {
...
app.get("/api/trash", async (req, res) => {
  try {
    ...
    // Get user's deleted emails
    const userDeletions = await EmailDeletion.find({ userId: userId }).sort({
      deletedAt: -1,
    });
    const deletedEmailIds = userDeletions.map((deletion) => deletion.emailId);
...
app.delete("/api/trash/:id/permanent", async (req, res) => {
  try {
    ...
    if (!existingPermanentDeletion) {
      const permanentDeletionCount =
        await PermanentEmailDeletion.countDocuments();
...
app.delete("/api/trash/empty", async (req, res) => {
  try {
    ...
    const permanentDeletions = deletedEmailIds.map((emailId, index) => ({
      id: `permanent-deletion-${Date.now()}-${
        permanentDeletionCount + index + 1
      }`,
      emailId: emailId,
      userId: userId,
      permanentlyDeletedAt: new Date(),
    ...
    const newPermanentDeletions = permanentDeletions.filter(
      (pd) => !existingEmailIds.includes(pd.emailId)
    );
    ...
    res.json({
      ...
      deletedCount: userDeletions.length,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete("/api/distribution-lists/:id", async (req, res) => {
  try {
    const deletedList = await DistributionList.findOneAndDelete({
      id: req.params.id,
    });
    if (!deletedList) {
      return res.status(404).json({ error: "Distribution list not found" });
...
const requireAdminAccess = async (req, res, next) => {
  try {
    ...
    if (!["admin", "manager", "team-leader"].includes(adminUser.role)) {
      console.log(
        `[Admin Middleware] Insufficient permissions. User role: ${adminUser.role}`
      );
      return res
        .status(403)
        .json({ error: "Insufficient permissions for admin access" });
    }

    console.log(
      `[Admin Middleware] Access granted for user: ${adminUserId}, role: ${adminUser.role}`
    );
...
app.get("/api/admin/emails", requireAdminAccess, async (req, res) => {
  try {
    ...
    console.log(
      `[Admin API Request] /api/admin/emails - page: ${page}, limit: ${limit}, sortBy: ${sortBy}, sortOrder: ${sortOrder}`
    );
    ...
    console.log(
      `[Admin API Response] Returning ${emails.length} emails out of ${totalCount} total`
    );
...
app.get("/api/sessions/active", async (req, res) => {
  try {
    ...
    if (
      adminUser.role !== "admin" &&
      adminUser.role !== "manager" &&
      adminUser.role !== "team-leader"
    ) {
      return res.status(403).json({
        error:
          "Only administrators, managers, and team leaders can view active sessions",
      });
    }
    ...
    const allActiveSessions = await ActiveSession.find({ isActive: true }).sort(
      { loginTime: -1 }
    );
    ...
    const orphanedSessions = allActiveSessions.filter(
      (session) => !validUserIds.includes(session.userId)
    );

    const staleSessions = allActiveSessions.filter(
      (session) => new Date(session.lastActivity) < thirtyMinutesAgo
    );

    if (orphanedSessions.length > 0 || staleSessions.length > 0) {
      const totalCleanup = orphanedSessions.length + staleSessions.length;
      console.log(
        `Found ${totalCleanup} sessions to clean up (${orphanedSessions.length} orphaned, ${staleSessions.length} stale)`
      );
...
app.post("/api/sessions/active", async (req, res) => {
  try {
    const { userId, userName, userEmail, userRole, userDepartment, socketId } =
      req.body;
    ...
    if (!existingUser) {
      return res.status(404).json({
        error: "User not found in system. Cannot create active session.",
      });
    }
...
const cleanupStaleSessions = async () => {
  try {
    ...
    if (staleSessions.length > 0) {
      console.log(
        `Scheduled cleanup: Found ${staleSessions.length} stale sessions to clean up`
      );
      ...
      console.log(
        `Scheduled cleanup: Deleted ${result.deletedCount} stale sessions`
      );
    }
  } catch (error) {
